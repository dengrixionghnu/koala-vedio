    ...
    actions: {
           ...
           // 获取账户信息(登录)
            async getUserAccount(): Promise<any> {
                const res = await get(apiConfig.USER.GET_ACCOUNT, {}, true)
      ...
      // 获取用户信息
            async getUserDetail(id: number): Promise<{ data: UserInfo; type: string } | undefined> {
                  const resJson = await get(apiConfig.USER.GET_USER_DETAIL, { uid: id }, true)
      ...
      // 获取用户歌单 (可选登录)
            async getUserPlaylist(id: number, limit: number = 30, offset: number = 0): Promise<any> {
                const resJson = await get(apiConfig.USER.GET_USER_PLAYLIST, { uid: id, limit, offset }, true)
            ...
            async getArtistDetail(id: number): Promise<any> {
                const resJson = await get(apiConfig.ART.GET_ARTIST_DETAIL, { id })
                const data: any = {
                // 用户名
            ...
            async getArtistSong(id: number): Promise<any> {
                const resJson = await get(apiConfig.ART.GET_ARTIST_SONG, { id })
            ...
            async getSimiArtist(id: number): Promise<any> {
                const resJson = await get(apiConfig.ART.GET_ARTIST_SIMI, { id })
            ...
            async getArtistAlbum(id: number): Promise<any> {
                const resJson = await get(apiConfig.ART.GET_ARTIST_ALBUM, { id })
            ...
            async getArtistFans(id: string): Promise<any> {
                const resJson = await get(apiConfig.ART.GET_ARTIST_FANS, { id })
                return {
                data: {
                    // 粉丝
