...
const userMainStore = createStore({
    ...
    actions:{
            ...
            async getHomePage(offset: number): Promise<any> {
                const res = await get(apiConfig.GET_HOME_PAGE, { cursor: { offset }, refresh: '' }, true, false)
            ...
            async getHomeBall(): Promise<Array<any>> {
                const res = await get(apiConfig.GET_HOME_BALL, {}, true, false)
                return res.data
            },
               /**
             *  ↓ 歌单
            ...
            async getSubscribe(id: number, t: number): Promise<any> {
                const args: any = { id, t }
                return await get(apiConfig.PLAYLIST.GET_PLAYLIST_SUBSCRIBE, args, true, false)
            },

            // 歌单-获取所有歌单分类
            async getPlaylistCatlist(): Promise<any> {
                return await get(apiConfig.PLAYLIST.GET_PLAYLIST_CATLIST, {})
            },
                // 歌单-今日推荐, 不支持offset
            async getRcmdPlaylist(limit: number = 6): Promise<{ data: Array<any>; more: boolean }> {
                const resJson = await get(apiConfig.PLAYLIST.GET_RECOMMENDS, { limit })
            ...
            async getPlaylistTop({
                ...
                const resJson = await get(apiConfig.PLAYLIST.GET_PLAYLIST_TOP, { limit, offset, cat })
            ...
            async getPlaylistHq({ limit = 6, before = undefined }): Promise<{
                ...
                const resJson = await get(apiConfig.PLAYLIST.GET_PLAYLIST_HQ, args)
            ...
            async getOfficialPlaylist({
                ...
                const resJson = await get(apiConfig.USER.GET_USER_PLAYLIST, { uid: 1463586082, limit, offset })
            ...
            async getPlaylistDetail(id: number): Promise<Playlist | undefined> {
                const data = await get(apiConfig.PLAYLIST.GET_PLAYLIST_DETAIL, { id }, true, true)
            ...
            async getToplistDetail(): Promise<any> {
                const resJson = await get(apiConfig.GET_TOPLIST_DETAIL, {})
            ...
            async getSongsDetail(trackIds: Array<number>): Promise<Array<Song>> {
                ...
                const requestQueue = idsChunks.map(idsArr => {
                    return get(apiConfig.SONG.GET_SONG_DETAIL, { ids: idsArr.toString() })
                })
            ...
            async getSongUrl(id: number): Promise<string | undefined> {
                const resJson = await get(apiConfig.SONG.GET_SONG_URL, { id })
            ...
            async getComment(
                ...
                return await get(apiConfig.COMMENT.GET_COMMENT_NEW, args, true, true)
            },
                /**
             * 点赞评论（登录）
             * @param id 资源 id
            ...
            async getCommentLike(id: number, cid: number, t: number, type: number): Promise<any> {
                ...
                return await get(apiConfig.COMMENT.GET_COMMENT_LIKE, args, true, false)
            },
                /**
             * 楼层评论
            ...
            async getCommentFloor(
                ...
                return await get(apiConfig.COMMENT.GET_COMMENT_FLOOR, args, false, false)
            }
